...
@tbl-stations shows the three stations used which are chosen to cover different
...
$+108h$ and $+120h$ which correspond to  $+4$ days and $12$ hours ($1200$ UTC) and
$+5$ days ($0000$ UTC) to highlight possible differences in the statistical
characteristic for different times of the day (noon/midnight).
...
($log(\mathrm{sd}(ens))$) are calculated which serve as the independent
...
variable used to train the models.
...
As described by @lang2020 in detail, the response (observed near-surface temperature; $t_{2m}$)
is assumed to follow a Gaussian distribution $\mathcal{N}$ which is defined by the
two parameters $\mu$ (location) and $\sigma$ (scale).

$$
t_{2m} \sim \mathcal{N}(\mu, \sigma)
$$

Both parameters can be expressed by an additive predictor $\eta_\bullet$. In the classical
non-homogeneous Gaussian regression the following specification is used frequently:

$$
\mu = \eta_\mu = \beta_0 + \beta_1 \overline{\mathmf{ens}} and
$$

$$
log(\sigma) = \eta_\sigma = \gamma_0 + \gamma_1 log(\mathmf{sd}(\mathmf{ens}))
$$


where 

RETO RETO RETO RETO RETO RETO RETO RETO RETO RETO
RETO RETO RETO RETO RETO RETO RETO RETO RETO RETO
RETO RETO RETO RETO RETO RETO RETO RETO RETO RETO
RETO RETO RETO RETO RETO RETO RETO RETO RETO RETO
RETO RETO RETO RETO RETO RETO RETO RETO RETO RETO
...
While the location parameter drives the expectation, the scale defines the
width of the distribution. Bo




Both parameters are expressed by a linear predictor
including a seasonal varying intercept ($\beta_0$, $\gamma_0$) and seasonal varying
slope coefficient ($\beta_1$, $\gamma_1$), all of which are smooth cyclic P-splines
and allow to adjust for different statistical characteristics over the course of the year.
