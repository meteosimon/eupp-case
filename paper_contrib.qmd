...
(hacky phase) post-processing benchmark data set for post-processing near surface temperature.
...
with smoothly varying seasonal effects. For demonstration, the method is only
...
the ensemble standard deviation ($\text{sd}(\text{sd})$) drives the scale of
...
relation ship between the response and the 
...
To allow the statistical model to depict such seasonal dependencies, the two
additive predictors are extended to:
...
All regression coefficients are now smooth functions which depend on the day of the year
($f(\text{yday}$); cyclic P-splines). Conceptually, this is similar to using a rolling
window training period (e.g., $\pm30$days).
In an ideal world where the ensemble forecasts are prefect, $\beta_0$/$\gamma_0$ would be $0.0$,
...
This indicates that the ensemble is able to accurately predict the expected temperature
via the mean of the ensemble. The effects for the log-scale ($\log(\sigma)$; $\gamma_0$, $\gamma_1$)
...
```{r plot_vertical_all}
#| label: fig-effects
#| fig-cap: Estimated seasonal varying effects $\beta_0()$, $\beta_1()$, $\gamma_0()$, and
#|          $\gamma_1()$ (top down) for the two forecast steps $+108h$ (noon) and $+120h$ (midnight).
